...
const GardenListScreen = ({ navigation }) => {
    ...
    // ALTERAÇÃO: A função de busca de hortas volta a ser um .select() simples
    ...
    // ALTERAÇÃO: A função de criar horta volta a ser um .insert() simples
    ...
    // Nenhuma alteração aqui, já estava usando .delete() direto
...
const GardenScreen = ({ route, navigation }) => {
    ...
    // Estado do modal de nova planta
    ...
    // Estado e ações para os botões adicionais no cabeçalho da FlatList
    const [irrigationStatus, setIrrigationStatus] = useState('off');
    const [inviteModalVisible, setInviteModalVisible] = useState(false);

    const handleToggleIrrigation = () => {
        setIrrigationStatus((prev) => (prev === 'on' ? 'off' : 'on'));
    };

    // Carregamento de dados e subscription
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`garden-plants-channel-${gardenId}`)
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'plants', filter: `garden_id=eq.${gardenId}` },
                () => fetchPlants(gardenId)
            )
            .subscribe();
    ...
    const PlantDetailScreen = ({ plant, onClose }) => (
        <View style={styles.pageContainer}>
            <View style={styles.catalogHeader}>
                <Text style={styles.pageTitle} numberOfLines={1}>{plant.name}</Text>
            ...
            <ScrollView>
                <Card style={styles.detailCard}>
                    ...
                    <Card.Content style={styles.detailContent}>
                        <Text style={styles.catalogScientificName}>{plant.scientific_name}</Text>
                    ...
                    <TouchableOpacity onPress={() => onSelectPlant(item)}>
                        <Card style={styles.catalogCard}>
                            ...
                            <Card.Content style={styles.catalogContent}>
                                ...
                                <Text style={styles.catalogDescription} numberOfLines={2}>{item.description}</Text>
                            </Card.Content>
                        </Card>
                    </TouchableOpacity>
                )}
    ...
    if (selectedPlant) {
        return <PlantDetailScreen plant={selectedPlant} onClose={() => setSelectedPlant(null)} />;
    }
    if (isPestsVisible) {
        return <PestsScreen pests={pests} onClose={() => setIsPestsVisible(false)} />;
    }
    if (isCatalogVisible) {
        return (
            <CatalogScreen
    ...
    return (
        <SafeAreaView style={styles.containerFull}>
            ...
            <FlatList
                ListHeaderComponent={<>
                    ...
                    <View style={styles.featuresGrid}>
                        <TouchableOpacity style={styles.featureButton} onPress={() => setIsCatalogVisible(true)}>
                            <Text style={styles.featureText}>Catálogo</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.featureButton} onPress={() => setIsPestsVisible(true)}>
                            <Text style={styles.featureText}>Pragas</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                            style={[styles.featureButton, { backgroundColor: irrigationStatus === 'on' ? '#C8E6C9' : '#FFFFFF' }]}
                            onPress={handleToggleIrrigation}
                        >
                            <Text style={styles.featureText}>{irrigationStatus === 'on' ? 'Desligar Irrigação' : 'Irrigar Agora'}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.featureButton} onPress={() => setInviteModalVisible(true)}>
                            <Text style={styles.featureText}>Convidar</Text>
                        </TouchableOpacity>
                    </View>
            ...
            <Modal visible={inviteModalVisible} transparent={true} animationType="fade" onRequestClose={() => setInviteModalVisible(false)}>
                <TouchableOpacity style={styles.pickerBackdrop} onPress={() => setInviteModalVisible(false)}>
                    <View style={styles.pickerContainer}>
                        <Card style={styles.inviteCard}>
                            <Card.Content>
                                <Text style={styles.inviteText}>Convide alguém para colaborar nesta horta.</Text>
                                <View style={styles.inviteActions}>
                                    <TouchableOpacity style={[styles.inviteButton, styles.rejectButton]} onPress={() => setInviteModalVisible(false)}>
                                        <Text style={styles.inviteButtonText}>Cancelar</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={[styles.inviteButton, styles.acceptButton]} onPress={() => setInviteModalVisible(false)}>
